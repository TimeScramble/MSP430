...
def main():
    ...
    sxfreq = Slider(ax_slider_freq, 'Sampling rate', 1, 4, valinit=4, valstep=1)
    ...
    while not stop_event.is_set():
        ...
        #saved_time = dates.date2num(test_date)
        ...
        if not stop_reading.is_set():
            ...
            if len(numbers) == 5:  # save values in graph_parameters
                var3.append(numbers[2])
                var4.append(numbers[3])
                var5.append(numbers[4])
        ...
        draw(dates.date2num(time_axis), var3, var4, var5, stop_reading)
...
def draw(time, v3, v4, v5, stop_reading):
    """
    DRAWING
    """
    #print(time)
